          ...
          DefaultButton(
            ...
            press : () async {
              if (_formKey.currentState.validate()) {
                ...
                http.Request checkUser = await getUser();
                checkUser.body.contains('success') ?
  ...
  Future<http.Response> userLogin(username, password) async{
    ...
    try {
      ...
      if(response.statusCode == HttpStatus.ok){
        setUpLocator();
        String userKey = await LocalStorageService().setId();
        try{
          void writingToDisk = LocalStorageService().saveStringToDisk(userKey, username);
        }catch (e){
          print("Error while writing key:username in cache: " + e);
        }
  ...
  Future loginAuth() async {
    ...
    try {
      ...
      if (form.validate()) {
        ...
        // this should not come ig, instead getUser(?)
        Response response = await dio.post(Urls.login, data: {
          'username': username,
          'password': password,
        });

        // LocalStorageService().usernameKey = await LocalStorageService().setId();
        // storage.saveStringToDisk(LocalStorageService().usernameKey, username);
        resp = json.decode(response.toString());
        if (resp.containsKey('success')) {
          await userCache.write(resp.keys);
          print(resp);
          print("Response data : " + response.data);
          print("Login success");
          return LoginSuccessScreen.routeName;
        }else {
          hasError = true;
